    ...
    if (params.is_causal) {
        ...
        VCOLMAJOR_SWITCH(params.v_dim_stride != 1, V_colmajor_, [&] {
            ...
            VARLEN_SWITCH(params.cu_seqlens_q || params.cu_seqlens_k || params.seqused_q || params.seqused_k || params.leftpad_k, Varlen, [&] {
                ...
                bool const has_qv_runtime = params.qv_ptr && Arch == 90 && !Is_FP8 && kHeadDim == 64 && kHeadDimV >= 256;
                if constexpr (Varlen) {
                    bool const append_kv_runtime = params.knew_ptr;
                    if (append_kv_runtime) {
                        if (has_qv_runtime) {
                            if constexpr (Enable_cluster && !PagedKVNonTMA) {
                                bool const use_cluster_runtime = (cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0);
                                if (use_cluster_runtime) {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 2, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, true, PackGQA, Split, V_colmajor>(params, stream);
                                } else {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, true, PackGQA, Split, V_colmajor>(params, stream);
                                }
                            } else {
                                run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, true, PackGQA, Split, V_colmajor>(params, stream);
                            }
                            ...
                            if constexpr (Enable_cluster && !PagedKVNonTMA) {
                                bool const use_cluster_runtime = (cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0);
                                if (use_cluster_runtime) {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 2, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, false, PackGQA, Split, V_colmajor>(params, stream);
                                } else {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, false, PackGQA, Split, V_colmajor>(params, stream);
                                }
                            } else {
                                run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, true, false, PackGQA, Split, V_colmajor>(params, stream);
                            }
                        ...
                        if (has_qv_runtime) {
                            if constexpr (Enable_cluster && !PagedKVNonTMA) {
                                bool const use_cluster_runtime = (cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0);
                                if (use_cluster_runtime) {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 2, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, true, PackGQA, Split, V_colmajor>(params, stream);
                                } else {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, true, PackGQA, Split, V_colmajor>(params, stream);
                                }
                            } else {
                                run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, true, PackGQA, Split, V_colmajor>(params, stream);
                            }
                            ...
                            if constexpr (Enable_cluster && !PagedKVNonTMA) {
                                bool const use_cluster_runtime = (cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0);
                                if (use_cluster_runtime) {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 2, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, false, PackGQA, Split, V_colmajor>(params, stream);
                                } else {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, false, PackGQA, Split, V_colmajor>(params, stream);
                                }
                            } else {
                                run_flash_fwd<Arch, kHeadDim, kHeadDimV, 1, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, false, false, PackGQA, Split, V_colmajor>(params, stream);
                            }
